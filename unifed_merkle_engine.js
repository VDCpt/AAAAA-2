...
window.gerarMasterHashFinal = async function(colecaoEvidencias) {
    ...
    if (typeof CryptoJS !== 'undefined' && CryptoJS.SHA256) {
        // CIR-01: .toUpperCase() garante uniformidade com sha256Hash() interno (64 chars UPPERCASE)
        // Sem esta normalização, comparações H1===H2 falhariam em auditorias independentes.
    ...
    try {
        ...
        const hashBuffer = await window.crypto.subtle.digest('SHA-256', msgBuffer);
        const hashArray = Array.from(new Uint8Array(hashBuffer));
        // CIR-01b: .toUpperCase() — uniformidade com sha256Hash() interno e path CryptoJS
