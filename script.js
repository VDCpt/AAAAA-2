...
function activateDemoMode() {
    ...
    window._demoModeTimer = setTimeout(async () => {
        try {
            ...
            // CIR-02: faturaPlataforma (BTF) adicionado por mês.
            // CAUSA RAIZ: sem este campo, seriesMensais (L6521) calculava Math.abs(despesas - 0)
            // em vez de Math.abs(despesas - btfMensal), reventando o desvio-padrão do Z-Score IC99%
            // e produzindo a discrepância 534,15 € (Prova Rainha) vs 83,98 € (Simulação Macroeconómica).
            // Distribuição BTF: total 262.94 € confirmado por UNIFEDSystem.documents.invoices.totals.invoiceValue.
            // Set 2024: 0.00 € (extrato sem fatura); Out: 85.12 €; Nov: 90.95 €; Dez: 86.87 €
            // Soma: 0.00 + 85.12 + 90.95 + 86.87 = 262.94 € ✓
...
function performForensicCrossings() {
    ...
    // ── mediaMensalReal: discrepância mensal média do operador (fonte auditável) ──
    // Fórmula: (BTOR − BTF) / mesesDados. Exposta em cross e em
    // UNIFEDSystem.analysis.mediaMensalReal para rastreabilidade directa
    // no bloco de conclusão (pure-wc-finding-4) sem divisão opaca por 38000.
    ...
    // ── Propagar mediaMensalReal para contrato de interface UNIFEDSystem.analysis ──
    ...
    return async function(system) {
        ...
        try {
            ...
            (function _fixTemplateTextLeak() {
                ...
                // ── PATCH P32+F4 — Substituição de cadeia opaca por variável rastreável ──
                // ANTERIOR (P32): _mediaMensalOmissao = (cross.impactoMensalMercado || 0) / 38000
                //   → cadeia: discrepanciaMensalMedia × 38000 / 12 / 38000 = discrepanciaMensalMedia / 12
                //   → opaca, não auditável directamente por perito externo.
                // CORRIGIDO (F4): window.UNIFEDSystem.analysis.mediaMensalReal
                //   → campo explícito populado em performForensicCrossings()
                //   → fórmula: (BTOR − BTF) / mesesDados — directamente rastreável.
                //   → fallback: cross.mediaMensalReal || 0 se analysis ainda não sincronizou.
                const _mediaMensalReal = (system.analysis && system.analysis.mediaMensalReal != null)
                    ? system.analysis.mediaMensalReal
                    : (cross.mediaMensalReal || 0);
                const _danoMensalFmt = window.formatForensicCurrency
                    ? window.formatForensicCurrency(_mediaMensalReal)
                    : fmt(_mediaMensalReal);
